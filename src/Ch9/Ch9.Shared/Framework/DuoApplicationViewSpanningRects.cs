...
namespace Ch9.Views
{
	...
	{
		public static bool ForcedDualMode { get; set; } = true;
		...
		public IReadOnlyList<Rect> GetSpanningRects()
		{
			if (ContextHelper.Current is Activity currentActivity)
			{
				...
				{
					if (ForcedDualMode || _helper.IsDualMode)
					{
						...
						var occludedRects = wuxWindowBounds.Width > 1350 + 100
							? new[] { new Rect(wuxWindowBounds.Width / 2, wuxWindowBounds.Top, 40, wuxWindowBounds.Height) }
							: new Rect[0];
						//: _helper
						//	.DisplayMask
